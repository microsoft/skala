...
from skala_model.utils.pad_ragged import pad_ragged, unpad_ragged
...
class SkalaFunctional(ExcFunctionalBase):
    ...
    def dtype(self) -> torch.dtype:
        input_layer = self.input_model[0]
        assert isinstance(input_layer, nn.Linear)
        return input_layer.weight.dtype
...
class NonLocalModel(nn.Module):
    ...
    def forward(
        ...
        result = self.concat_layer(features)
...
class O3Linear(nn.Module):
    ...
    def weight_list(self) -> list[torch.Tensor]:
        result = []
        for i_in, i_out in self.instr:
            weight = getattr(self, f"weight_{i_in}_{i_out}")
