...
extern "C" torch_tensor_t skala_tensor_load(const char *filename) {
  ...
  input.close();
  auto tensor =
      torch::jit::pickle_load(bytes).toTensor().to(torch::Device(torch::kCPU));
  return new torch::Tensor(std::move(tensor));
}

extern "C" torch_tensor_t skala_tensor_sum(const torch_tensor_t tensor) {
  auto t = reinterpret_cast<torch::Tensor *>(tensor);
  auto sum = t->sum();
  return new torch::Tensor(std::move(sum));
}

extern "C" double skala_tensor_item_double(const torch_tensor_t tensor) {
  auto t = reinterpret_cast<torch::Tensor *>(tensor);
  return t->item<double>();
}

extern "C" torch_jit_script_module_t skala_model_load(const char *filename,
  ...
  if (std::stoi(extra_files.at("protocol_version")) != 2) {
    std::string message =
        "Unsupported protocol version " + extra_files.at("protocol_version");
    ctorch_error(message, [&]() { delete module; });
  }

  auto feature_str = extra_files.at("features");
  // formatted as ["feature1", "feature2", ...] parse without using a full json
  // library
...
skala_model_forward(torch::jit::script::Module module,
                    const c10::Dict<std::string, at::Tensor> &features) {
...
extern "C" void skala_model_get_exc(torch_jit_script_module_t module,
                                    skala_dict_t input,
...
extern "C" void skala_model_get_exc_and_vxc(torch_jit_script_module_t module,
                                            skala_dict_t input,
                                            torch_tensor_t *exc_output,
                                            skala_dict_t *grad_output) {
  auto model = static_cast<torch::jit::script::Module *>(module);
  auto dict = static_cast<SkalaDict *>(input);
  ...
  for (const auto &entry : *dict) {
    ...
    bool requires_grad = (key == "density" || key == "grad" || key == "kin" ||
...
extern "C" skala_dict_t skala_dict_new() {
  SkalaDict *input = new SkalaDict();
  return static_cast<skala_dict_t>(input);
}

extern "C" void skala_dict_insert(skala_dict_t input, const char *key,
                                  const torch_tensor_t *values, size_t size) {
  auto dict = static_cast<SkalaDict *>(input);
  std::vector<at::Tensor> tensors;
...
extern "C" skala_list_t skala_dict_at(skala_dict_t input, const char *key) {
  auto dict = static_cast<SkalaDict *>(input);
  if (!dict->contains(key)) {
    std::string message =
        "Key '" + std::string(key) + "' not found in SkalaDict";
...
extern "C" size_t skala_list_size(skala_list_t input) {
  auto list = static_cast<SkalaList *>(input);
  return list->size();
}

extern "C" torch_tensor_t skala_list_at(skala_list_t input, size_t index) {
  auto list = static_cast<SkalaList *>(input);
  if (index >= list->size()) {
    std::string message = "Index " + std::to_string(index) +
                          " out of bounds for SkalaList of size " +
...
extern "C" torch_tensor_t skala_tensor_mul(const torch_tensor_t a,
                                           const torch_tensor_t b) {
...
extern "C" torch_tensor_t skala_tensor_mean(const torch_tensor_t tensor) {
  auto t = reinterpret_cast<torch::Tensor *>(tensor);
  auto m = t->mean();
  return new torch::Tensor(std::move(m));
}

extern "C" void *skala_tensor_data_ptr(const torch_tensor_t tensor) {
  auto t = reinterpret_cast<torch::Tensor *>(tensor);
...
extern "C" int64_t skala_tensor_ndim(const torch_tensor_t tensor) {
  auto t = reinterpret_cast<torch::Tensor *>(tensor);
  return t->ndimension();
}

extern "C" int64_t skala_tensor_size(const torch_tensor_t tensor, int64_t dim) {
  auto t = reinterpret_cast<torch::Tensor *>(tensor);
  return t->size(dim);
}

extern "C" int64_t skala_tensor_numel(const torch_tensor_t tensor) {
  auto t = reinterpret_cast<torch::Tensor *>(tensor);
  return t->numel();
}

extern "C" void skala_dict_delete(skala_dict_t input) {
  if (input == nullptr)
    return;
  auto dict = static_cast<SkalaDict *>(input);
  delete dict;
...
extern "C" void skala_list_delete(skala_list_t input) {
  if (input == nullptr)
    return;
  auto list = static_cast<SkalaList *>(input);
  delete list;
