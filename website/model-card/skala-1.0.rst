...
#. The SCF implementation provided uses PySCF, which runs the functional on CPU. We also provide a traced version of the Skala-1.0 functional so that other, more optimized open-source SCF codes can integrate it into their pipelines. The standalone SkalaXC library provides C++, C, and Fortran integration.
