...
#include "host/collective_error.hpp"
...
double SkalaDeviceDriver::eval_exc_vxc_uks(
  ...
  double exc_value = 0.0;
  mpi::collective_try(lb_.runtime(), [&] {
    activate_cuda_device(device_id_);
    ...
    device_data->allocate_static_data_exc_vxc(basis.nbf(), basis.nshells(),
                                              terms, true);
    ...
    FeatureDict local_potentials =
        model_grid_exchange_->prepare_local_potentials(needs_gradient, is_mgga,
                                                       model_device);
    at::Tensor exc_value_device = torch::zeros(
        {1},
        torch::TensorOptions().dtype(torch::kFloat64).device(model_device));
    ...
    for (const auto& batch : model_grid_exchange_->local_batches()) {
      ...
      auto exc = evaluate_cuda_model_energy(*model_, features, device_id_,
                                            batch_index);
    ...
    while (task_it != tasks.end()) {
      ...
      for (const auto density : {GauXC::DEN_S, GauXC::DEN_Z}) {
        zmat_skala_vxc(aos_data->host_device_tasks.size(), max_basis,
                       max_points, aos_data->aos_stack.device_tasks,
                       terms.xc_approx, density, device_data->queue());
        if (is_mgga)
    ...
    exc_value = exc_value_device.item<double>();
  });
#ifdef GAUXC_HAS_MPI
  ...
  mpi::collective_try(lb_.runtime(), [&] {
    ...
    for (std::size_t batch_index = 0; batch_index < local_batches.size();
      ...
      FeatureDict features = model_grid_exchange_->prepare_local_batch_features(
          batch, local_features, lb_.molecule(), model_->feature_keys(),
          model_device, true);
      auto energy = evaluate_cuda_model_energy(*model_, features, device_id_,
                                               batch_index);
      finite_checks.push_back(
          {"model energy in batch " + std::to_string(batch_index),
      ...
      auto dE_dw = model_dE_dw(features, device_id_, batch.point_count.raw(),
                               batch_index);
      finite_checks.push_back(
    ...
    while (task_it != tasks.end()) {
      ...
      for (const auto& task : aos_data->host_device_tasks)
        max_points = std::max(max_points, static_cast<std::int32_t>(task.npts));
      if (needs_gradient)
        transform_skala_vxc_for_grad(
            aos_data->host_device_tasks.size(), max_points,
            aos_data->aos_stack.device_tasks, device_data->queue());

      if (is_mgga)
        device_lwd->inc_exc_grad_mgga(device_data.get(), GauXC::UKS, false,
                                      true);
      else if (model_->is_gga())
