...
In pursuit of the universal functional for density functional theory (DFT), the OneDFT team from Microsoft Research AI for Science has developed the Skala exchange-correlation functional, as introduced in `Accurate and scalable exchange-correlation with deep learning (arXiv v5), Luise et al. 2025 <https://arxiv.org/abs/2506.14665v5>`__.
...
On GMTKN55, which covers general main-group thermochemistry, kinetics, and noncovalent interactions, the Skala functional achieves a WTMAD-2 of 3.89 kcal/mol.
...
#. The Skala functional is shared with the research community to facilitate reproduction of the evaluations presented in our paper.
#. Evaluating reaction energy differences by computing the total energy of all compounds in a reaction using a self-consistent field (SCF) calculation with the Skala exchange-correlation functional.
#. Evaluating the total energy of a molecule using an SCF calculation with the Skala exchange-correlation functional. Note that, as with all density functionals, energy differences are predicted much more reliably than total energies of individual molecules.
#. The SCF implementation provided uses PySCF, which runs the functional on CPU. We also provide a traced version of the Skala functional so that other, more optimized open-source SCF codes—including GPU-enabled ones—can integrate it into their pipelines, for instance through GauXC. A compatible fork of GauXC is included in this repository.
...
#. The Skala functional is trained on atomization energies, conformers, proton affinities, ionization potentials, elementary reaction pathways, and non-covalent interactions, as well as a small amount of electron affinities and total energies of atoms. We have benchmarked performance on W4-17 for atomization energies and on GMTKN55, which covers general main-group thermochemistry, kinetics, and noncovalent interactions, to provide an indication of generalization beyond the training set. We have also evaluated robustness on dipole moment predictions and geometry optimization.
#. The Skala functional has been trained on data containing the following elements: H–Ar, Br, Kr, I, Xe. It has been tested on data containing H–Ca, Ge–Kr, Sn–I, Pb, and Bi.
...
#. Skala will also be available through `Azure AI Foundry <https://labs.ai.azure.com/projects/skala/>`__, where it is coupled with Microsoft's GPU-accelerated `Accelerated DFT <https://arxiv.org/abs/2406.11185>`__ application.
...
- Total energies, electron affinities, and ionization potentials (up to triple ionization) for atoms from H to Ar (excluding Li and Be due to basis set constraints).
  ...
  Extrapolation of the correlation energy was performed by fitting a :math:`Z^{-3}` expression, while the Hartree–Fock energy was extrapolated using the two-point scheme.\ :footcite:`karton2006`
...
#. The dipole benchmark dataset.\ :footcite:`hait2018`
#. Conformer search benchmark dataset of 22 molecules spanning 24 to 176 atoms, used for cost-scaling analysis.\ :footcite:`grimme2019`
...
On W4-17, the Skala functional predicts atomization energies at chemical accuracy (:math:`\sim`\ 1 kcal/mol MAE).
...
Finally, the scaling results show that the Skala functional exhibits the asymptotic scaling behavior of a meta-GGA functional, with an approximate prefactor of 3 relative to r2SCAN.
