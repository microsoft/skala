...
redistributed here: ``python -m skala_benchmark fetch-dataset`` downloads them from the original
...
.. code-block:: bash
  ...
  pixi run -e default python -m skala_benchmark fetch-dataset
   export OMP_NUM_THREADS=16

  pixi run -e default python -m skala_benchmark run benchmark-output \
    --env-id local-cpu \
...
.. code-block:: bash
  ...
  pixi run -e default python -m skala_benchmark run benchmark-smoke \
    --env-id cpu-smoke \
    --device cpu \
    --max-atoms 3 \
    --basis def2-svp
...
.. code-block:: bash
   ...
   python -m skala_benchmark collect benchmark-output
