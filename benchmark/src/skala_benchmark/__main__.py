...
from skala_benchmark.orchestrator import SweepRequest, parse_duration, run_sweep
from skala_benchmark.protocol import DEFAULT_PROTOCOL, Device


def main(argv: Sequence[str] | None = None) -> None:
    """Run one stage of the benchmark workflow."""
    parser = argparse.ArgumentParser(
        prog="python -m skala_benchmark",
    ...
    if args.command == "run":
        ...
        from skala_benchmark.fetch import main as fetch_main

        raise SystemExit(fetch_main(args.dataset_dir))
    elif args.command == "collect":
        from skala_benchmark.collect_results import collect_results

        output_dir = args.output_dir or str(Path(args.input_dir) / "collected")
        collect_results(args.input_dir, output_dir)
    else:
        from skala_benchmark.report.generate import generate
