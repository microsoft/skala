...
:mod:`skala.benchmark.fetch`; what is kept here is a manifest naming each
...
def read_manifest() -> list[ManifestEntry]:
    ...
    resource = files("skala.benchmark") / MANIFEST_FILE
...
def load_benchmark_molecules(
    ...
    if not path.exists():
        command = "python -m skala.benchmark fetch-dataset"
    ...
    if {str(record["hash"]) for record in records} != expected:
        raise ValueError(
            ...
            "benchmark; re-run 'python -m skala.benchmark fetch-dataset'"
        )
    return [
        BenchmarkMolecule(
