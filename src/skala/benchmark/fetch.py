...
``docs/benchmarks.rst`` for the datasets and the citations they require.

Every structure is checked against :mod:`skala.benchmark.dataset`'s manifest --
...
def fetch_dataset(dataset_dir: str | Path | None = None) -> Path:
    ...
    Args:
        dataset_dir: Directory to write the dataset to. Defaults to
            :func:`skala.benchmark.dataset.default_dataset_dir`.
