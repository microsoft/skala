...
def SkalaKS(
    ...
    *,
    ...
    if isinstance(xc, str):
        if with_dftd3:
            raise ValueError(
                f"DFT-D3 dispersion correction is not supported for native PySCF functionals (xc={xc!r}). "
                "Pass with_dftd3=False or use pyscf.dft.RKS/UKS directly."
            )
        result = _create_native_pyscf_ks(
            mol,
            xc,
