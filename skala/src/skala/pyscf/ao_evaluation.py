...
def _evaluate_feature_block(
    ...
    if compile_feature_function:
        result = torch.compile(feature_function.forward)(
            active_dm_submatrix, block.ao_values
        )
        assert isinstance(result, Tensor)
        return result
    result = feature_function(active_dm_submatrix, block.ao_values)
    assert isinstance(result, Tensor)
    return result
...
def _resolve_ao_block_size(
    ...
    result = block_size - block_size % dft.gen_grid.BLKSIZE
